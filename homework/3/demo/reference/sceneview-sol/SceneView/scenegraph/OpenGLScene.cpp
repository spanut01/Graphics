...
// begin ta code
// begin hack to make normals work with evan's shapes
#include "ta_shapes/Mesh.h"

OpenGLScene *singletonForNormals;

void OpenGLScene::renderNormalsForMesh(Mesh *mesh)
{
    singletonForNormals = this;
    mesh->drawNormals(shapesRenderNormal);
}

void OpenGLScene::shapesRenderNormal(const Vector4 &vertex, const Vector4 &direction)
{
    singletonForNormals->renderNormal(vertex, direction);
}
// end hack to make normals work with evan's shapes
// end ta code
