...
#define SETUP_ACTION(dock, key) \
    ...
    SETUP_ACTION(modelerDock,   "CTRL+5");
    SETUP_ACTION(rayDock,       "CTRL+6");
...
#undef SETUP_ACTION
    ...
    tabifyDockWidget(ui->brushDock, ui->modelerDock);
...
#define BIND(b) { DataBinding *_b = (b); m_bindings.push_back(_b); assert(connect(_b, SIGNAL(dataChanged()), this, SLOT(settingsChanged()))); }
    ...
    // Modeler dock
    BIND( ChoiceBinding::bindRadioButtons(NUM_OBJ_TOOLS, settings.objTool,
                                    ui->objFilmPlaneTranslate,
                                    ui->objLookVectorTranslate,
                                    ui->objTrackballRotate,
                                    ui->objUniformScale) )
...
void MainWindow::fileCopy3Dto2D()
{
    ...
    ui->canvas2D->setSize(ui->canvas3D->width(), ui->canvas3D->height());
