...
void recursiveRender(SceneNode* node, Matrix compositeMatrix)
{
	...
	for (int j=0; j<objectVec.size(); j++) {
		applyMaterial(objectVec[j]->material);
		renderShape(objectVec[j]->type);
	}
	glPopMatrix();

	std::vector<SceneNode*> childrenVec = node->children;
	for (int k=0; k<childrenVec.size(); k++) {
		recursiveRender(childrenVec[k], compositeMatrix);
	}
}
...
void myGlutDisplay(void)
{
	...
	Point p_temp(eyeX, eyeY, eyeZ);
	Vector v_temp(lookX, lookY, lookZ);
	Vector up_temp(0, 1, 0);

	camera->Orient(p_temp, v_temp, up_temp);
	...
	if (wireframe) {
		...
		recursiveRender(root, compositeMatrix);
	}
