...
/*  ============== Vertex ==============
	...
	When using with vertex arrays, you will need to define the 'stride' between
	each vertex, thus calculate the size of our 'vertex' struct and multiply it by
...
struct vertex{
	...
	float nx,ny,nz;		
	// texture coordinates (we choose 's' and 't', but could also be 'u' or 'v' 'w' as in the literature)
	float s,t,p,q;		
};
...
struct face{
	...
	// A face can have anywhere from 3 to n vertices using this structure.
	int vertexCount;
	...
	/*
		This destructor is causing a double free and needs to be improved.
	// Default destructor
	~face(){
		if(vertexCount != 0){
			delete[] vertexList;
