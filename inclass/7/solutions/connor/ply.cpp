...
void inline ply::addEdge(int va, int vb, int faceindex){
    int min, max;
    if(va<vb){
        min = va;
        max = vb;
    ...
    if(!e){
        ...
        while (e->vertices[1] != max && e->next != NULL){
            e = e->next;
        }
        if(e->vertices[1] == max){
            e->faces[1] = faceindex;
...
void ply::findEdges(){
    ...
    for(int i=0;i<faceCount;i++){
        struct face f = faceList[i];
        addEdge(f.vertexList[0], f.vertexList[1], i);
        addEdge(f.vertexList[1], f.vertexList[2], i);
        addEdge(f.vertexList[2], f.vertexList[0], i);
    }
}
...
void ply::renderSilhouette(){
    ...
    for(i=0; i<vertexCount; i++){
        ...
        while(e){
            if(faceList[e->faces[0]].dotProd * faceList[e->faces[1]].dotProd < 0.0f){
                vertex v1 = vertexList[e->vertices[0]];
                vertex v2 = vertexList[e->vertices[1]];
