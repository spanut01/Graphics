...
int ply::lookupEdge(int v1, int v2, int face){
    if (v1 > v2){
        return lookupEdge(v2, v1, face);
    }
    int len = edgeList.size();
    for (int i = 0; i < len; i++){
        edge* cur_edge = &edgeList[i];
        if (cur_edge->vertexes[0] == v1 && cur_edge->vertexes[1] == v2){
            //cout << "SKIP" << endl;
            cur_edge->faces[1] = face;
            return i;
        }
    }
    edge new_edge = edge();
    new_edge.vertexes[0] = v1;
    new_edge.vertexes[1] = v2;
...
void ply::findEdges(){
    for(int i = 0; i < faceCount; i++) {
        ...
        int* face_verts = curFace->vertexList;
        lookupEdge(face_verts[0], face_verts[1], i);
        lookupEdge(face_verts[1], face_verts[2], i);
        lookupEdge(face_verts[0], face_verts[2], i);
    }
...
void ply::renderSilhouette(){
    ...
    for (int i = 0; i < num_edges; i++){
        edge* cur_edge = &edgeList[i];
        face face0 = faceList[cur_edge->faces[0]];
        face face1 = faceList[cur_edge->faces[1]];
        //cout << "looking at an edge" << endl;
        if ((face0.dotProd > 0 && face1.dotProd < 0) ||
            (face0.dotProd < 0 && face1.dotProd > 0)){
            //cout << "drawing an edge" << endl;
            vertex v0 = vertexList[cur_edge->vertexes[0]];
            vertex v1 = vertexList[cur_edge->vertexes[1]];
