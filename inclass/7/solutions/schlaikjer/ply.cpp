...
void ply::findEdges(){
    ...
    edgeList = new struct baseEdge*[edgeCount];

    fprintf(stderr, "Model has %d vertexes\n", vertexCount);
    ...
    for (int i = 0; i < faceCount; i++){
        if (int err = this->addOrUpdateEdge(faceList[i].vertexList[0],
                                            faceList[i].vertexList[1],
                                            &faceList[i])){
            //fprintf(stderr, "Error a %d adding face %d edges %d, %d \n", err, i,
            //        faceList[i].vertexList[0],
            //        faceList[i].vertexList[1]);
        }
        if (int err = this->addOrUpdateEdge(faceList[i].vertexList[0],
                                            faceList[i].vertexList[2],
                                            &faceList[i])){
            //fprintf(stderr, "Error b %d adding face %d edges %d, %d \n", err, i,
            //        faceList[i].vertexList[0],
            //        faceList[i].vertexList[2]);
        }
        if (int err = this->addOrUpdateEdge(faceList[i].vertexList[2],
                                            faceList[i].vertexList[1],
                                            &faceList[i])){
            //fprintf(stderr, "Error c %d adding face %d edges %d, %d \n", err, i,
            //        faceList[i].vertexList[2],
            //        faceList[i].vertexList[1]);
        }
    }
    ...
    int unmatched = 0;
    int i;
    for (i=0; edgeList[i] != NULL; i++){
        if (edgeList[i]->faces[1] == NULL){
            unmatched++;
            fprintf(stderr, "Edge has vertices %d, %d and matches faces %p %p\n",
                    edgeList[i]->vertexes[0],edgeList[i]->vertexes[1],
                    edgeList[i]->faces[0], edgeList[i]->faces[1]);
        }
            
    }
    fprintf(stderr, "%d/%d faces are unmatched!\n", unmatched, i);
...
int ply::addOrUpdateEdge(int a, int b, face* triangle){
    for (int i=0; i < edgeCount && edgeList[i]; i++){

        if ((edgeList[i]->vertexes[0] == a && edgeList[i]->vertexes[1] == b)
        ||  (edgeList[i]->vertexes[1] == a && edgeList[i]->vertexes[0] == b)){

            if ((edgeList[i]->faces[0] && edgeList[i]->faces[0] == triangle)
             || (edgeList[i]->faces[1] && edgeList[i]->faces[1] == triangle)){
                return 1; //We are already in there somehow?
            }

            if (!edgeList[i]->faces[1]){
                edgeList[i]->faces[1] = triangle;
                return 0;
            } else {
                // Trying to add a 3rd face??
                return 2;
            }
            assert(1 == 0);
         }
    }
    // No existing matching edge, create new
    baseEdge* newEdge = new baseEdge(a, b, triangle);
    if (edgeInsertAt >= edgeCount){
        fprintf(stderr, "Adding more edges than expected! Realloc edge list\n");
        edgeList = (struct baseEdge**) realloc(edgeList, sizeof(struct baseEdge*) * edgeCount * 2);
        edgeCount *= 2;
        for (int ci=edgeInsertAt; ci < edgeCount; ci++){
            edgeList[ci] = NULL;
        }
    }

    edgeList[edgeInsertAt] = newEdge;
    edgeInsertAt++;
    return 0;
}
...
void ply::renderSilhouette(){
    ...
    for (int i=0; edgeList[i] != NULL && edgeList[i]->faces[1] != NULL; i++){
        float dotprod0 = edgeList[i]->faces[0]->dotProd;
        float dotprod1 = edgeList[i]->faces[1]->dotProd;

        if ((dotprod0 > 0.0 && dotprod1 < 0.0)
        ||  (dotprod0 < 0.0 && dotprod1 > 0.0)){
            vertex vert1 = vertexList[edgeList[i]->vertexes[0]];
            vertex vert2 = vertexList[edgeList[i]->vertexes[1]];
