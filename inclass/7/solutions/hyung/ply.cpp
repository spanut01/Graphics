...
void ply::face_check(int small, int big, int face){
  edge* min_edge = edgeList[small];


  while (min_edge != NULL){
    if (min_edge->vertexes[1] == big){
      min_edge->faces[1] = face;
      return;
    }
  ...
  new_edge->vertexes[0] = small;
  new_edge->vertexes[1] = big;
  
  new_edge->faces[0] = face;
  new_edge->faces[1] = -1;
  new_edge->next = edgeList[small];
  edgeList[small] = new_edge;
...
void ply::findEdges(){
    ...
    for (int i=0; i < faceCount; i++){
      ...
      int* curVertices = curFace.vertexList;
      
      int vertex1 = curVertices[0];
      int vertex2 = curVertices[1];
      int vertex3 = curVertices[2];

      int max = findMax(vertex1, vertex2, vertex3);
      int mid = findMid(vertex1, vertex2, vertex3);
      int min = findMin(vertex1, vertex2, vertex3);

      face_check(min, mid, i);
      face_check(min, max, i);
      face_check(mid, max, i);
...
void ply::renderSilhouette(){
    ...
    for (int i=0; i < vertexCount; i++){
      edge* cur_edge = edgeList[i];

      while (cur_edge != NULL){
        int face1 = cur_edge->faces[0];
        int face2 = cur_edge->faces[1];

        if (face2 != -1){
          if (faceList[face1].dotProd * faceList[face2].dotProd < 0){
            vertex v1 = vertexList[cur_edge->vertexes[0]];
            vertex v2 = vertexList[cur_edge->vertexes[1]];
